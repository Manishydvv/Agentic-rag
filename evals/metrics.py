...
@contextmanager
def dummy_span(name, **kwargs):
    logger.info(f"Starting: {name} | kwargs={kwargs}")
    yield
...
async def run_all_metrics(golden_dataset: dict, status_cb=None) -> dict:
    ...
    with dummy_span("🧪 Eval Phase 2 — All Metrics", total_samples=len(samples)):

        # ── Exp 1: Faithfulness ───────────────────────────────────────────────
        if status_cb:
            status_cb(f"🧪 Exp 1/6 — Faithfulness ({len(samples)} samples)...")
        with dummy_span("🧪 Exp 1 — Faithfulness"):
            inputs = [
                {
            ...
            logger.info(f"🧪 Faithfulness done | avg={round(df['faithfulness'].mean(), 3)}")
        ...
        with dummy_span("🧪 Exp 2 — Answer Relevancy"):
            inputs = [
            ...
            logger.info(f"🧪 Answer Relevancy done | avg={round(df['answer_relevancy'].mean(), 3)}")
        ...
        with dummy_span("🧪 Exp 3 — Context Precision"):
            inputs = [
                {
                    "user_input": s["question"],
            ...
            logger.info(f"🧪 Context Precision done | avg={round(df['context_precision'].mean(), 3)}")
        ...
        with dummy_span("🧪 Exp 4 — Context Recall"):
            inputs = [
                {
                    "user_input": s["question"],
                    "reference": s["reference"],
            ...
            logger.info(f"🧪 Context Recall done | avg={round(df['context_recall'].mean(), 3)}")
        ...
        with dummy_span("🧪 Exp 5 — Answer Correctness"):
            inputs = [
                {
                    "user_input": s["question"],
                    "response": s["actual_response"],
            ...
            logger.info(f"🧪 Answer Correctness done | avg={round(df['answer_correctness'].mean(), 3)}")
        ...
        with dummy_span("🧪 Exp 6 — Tool Correctness"):
            tool_rows = []
            for s in samples:
            ...
            logger.info(f"🧪 Tool Correctness done | avg={round(df['tool_correctness'].mean(), 3)}")
