...
def run_pipeline(golden_dataset: dict, progress_callback=None) -> dict:
    ...
    with dummy_span("🚀 Eval Phase 1 — Live Pipeline", total_samples=n):
        for i, sample in enumerate(samples):
            ...
            with dummy_span(
                ...
                try:
                    ...
                    raw_answer = data.get("response") or ""
                    thought_process = data.get("plan") or []
                    ...
                    logger.info(
                        f"✅ Response captured | tool={sample['actual_tools_called'][0]} "
                        f"chars={len(raw_answer)} contexts={len(sources)}"
                    )

                except requests.exceptions.ConnectionError:
                    logger.error("❌ Cannot reach FastAPI — is the app running on :8000?")
