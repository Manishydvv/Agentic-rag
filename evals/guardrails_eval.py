...
def _is_blocked(response_json: dict) -> bool:
    return response_json.get("source") == "guardrail"
...
def run_guardrails_eval(guardrails_samples: list, progress_callback=None) -> list:
    ...
    with dummy_span("🛡️ Eval — Guardrails Tests", total=n):
        for i, sample in enumerate(samples):
            ...
            with dummy_span(
                ...
                try:
                    ...
                    logger.error(f"❌ Guardrails test error: {e}")
                    blocked = False
                ...
                logger.info(
                        f"🛡️ {sample['result']} | expected_blocked={expected} "
                        f"actual_blocked={blocked} input={sample['input'][:60]}"
                )
